...
from numpy import clip, cos, radians, sin
...
def calculate_drag(
    ...
    rolling_resistances = (
        rolling_resistance_coefficient * vehicle_mass * gravity * cos(slope_radians)
    )
...
rpm_points = np.array([800, 1200, 1800, 2500, 3200, 4000, 4800, 5500, 6200])

torque_points = np.array([70, 110, 160, 200, 230, 260, 255, 235, 190])
...
app.layout = html.Div(
    [
        ...
        dcc.Dropdown(
            id="model_selector",
            options=[{"label": name, "value": name} for name in VEHICLE_MODELS.keys()],
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.Label(
                            "Przełożenia biegów (rozdzielone przecinkami, np. 3.6, 2.1, 1.4)"
                        ),
                        dcc.Input(
                ...
                html.Div(
                    [
                        ...
                        dcc.Graph(
                            id="torque_curve_graph", style={"marginBottom": "15px"}
                        ),
                        dcc.Button(
                            "Zapisz parametry napędu",
                            id="torque_calc_button",
                            n_clicks=0,
...
def update_parameters_from_model(selected_model_name):
    ...
    try:
        ...
        torque_array = np.array(
            [float(x) for x in model_data["torque_points"].split(",")]
        )

        gear_ratios_array = np.array(
            [float(x) for x in model_data["gear_ratios"].split(",")]
        )

        if len(rpm_array) != len(torque_array):
            raise ValueError(
                "Liczba punktów RPM i Momentu musi być równa i wynosić co najmniej 2."
            )

        interpolated_torque_f = scipy.interpolate.interp1d(
            rpm_array, torque_array, kind="cubic"
        )
        ...
        fig_curve.update_layout(
            template="plotly_white", xaxis_title="RPM", yaxis_title="Moment (Nm)"
        )
    except ValueError as e:
        fig_curve.add_annotation(
            text=f"Błąd danych: {e}", showarrow=False, font=dict(size=14, color="red")
        )
...
def update_torque(
    ...
    torque_str,
    wheel_radius_param,
    final_drive_ratio_param,
    gear_ratios_str,
    shift_up_rpm_param,
    shift_down_rpm_param,
):
...
def update_graph(
    ...
    fig_new.add_trace(
        go.Scatter(
            ...
            name="Prędkość",
            customdata=np.stack([rpms], axis=-1),
            hovertemplate="<b>Prędkość:</b> %{y:.1f} km/h<br>"
    ...
    if shifts:
        ...
        fig_new.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    size=10, color=shift_gears, line=dict(width=1, color="black")
                ),
                name="Zmiany biegów",
                hovertemplate="<b>Prędkość:</b> %{y:.1f} km/h<br>"
    ...
    fig_new.update_layout(
        template="plotly_white", xaxis_title="Czas (s)", yaxis_title="Prędkość (km/h)"
    )
